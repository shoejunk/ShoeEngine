...
TEST_F(ImageManagerTests, GetManagedType) {
    EXPECT_EQ(manager.GetManagedType(), "images");
}

TEST_F(ImageManagerTests, CreateFromJson) {
    EXPECT_TRUE(manager.CreateFromJson(testJson));
    
    const Image* image = manager.GetImage("test_image");
...
TEST_F(ImageManagerTests, GetNonexistentImage) {
    EXPECT_EQ(manager.GetImage("nonexistent"), nullptr);
}

TEST_F(ImageManagerTests, Clear) {
    EXPECT_TRUE(manager.CreateFromJson(testJson));
    EXPECT_NE(manager.GetImage("test_image"), nullptr);
    
    manager.Clear();
    EXPECT_EQ(manager.GetImage("test_image"), nullptr);
}
